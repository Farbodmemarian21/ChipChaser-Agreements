# ChipChaser Privacy Policy

**Last Updated: November 15, 2025**

## 1. Introduction

Bod Productions ("we," "us," or "our") operates the ChipChaser mobile application ("App"). We are committed to protecting your privacy and being transparent about how we collect, use, and safeguard your information.

This Privacy Policy explains:
- What information we collect from you
- How we use your information
- Who we share your information with
- Your privacy rights and choices
- How we protect your information

**Please read this Privacy Policy carefully.** By using ChipChaser, you agree to the collection and use of information in accordance with this policy. If you do not agree with our policies and practices, do not use the App.

**Contact Information:**  
Bod Productions  
Email: memariaf21@gmail.com

## 2. Information We Collect

### 2.1 Account Information (Collected via Clerk)

When you create an account, our authentication service provider Clerk collects and processes:

- **Email address** - Used for account creation, login, and important notifications
- **Username** - Your chosen display name
- **Password** - Encrypted and never stored in plain text
- **Authentication tokens** - Secure tokens that verify your identity
- **Account creation timestamp** - When you first registered

Clerk acts as our authentication service provider and processes this data on our behalf. For more information about how Clerk handles your authentication data, please review [Clerk's Privacy Policy](https://clerk.com/legal/privacy).

### 2.2 Profile Information

You may provide additional profile information:

- **Profile picture** - Optional photo uploaded by you, stored in our Amazon S3 servers
- **Display name** - Optional name different from your username
- **Bio or description** - Optional profile text

### 2.3 Poker Session Data

When you track poker sessions, you provide:

- **Buy-in amounts** - Money you brought to the game
- **Cash-out amounts** - Money you left with
- **Game dates and times** - When you played
- **Venue names/locations** - User-entered text descriptions of where you played (e.g., "Joe's House," "Local Card Room")
- **Game types** - Cash game, tournament, or other formats
- **Session notes** - Optional descriptions or comments
- **Win/loss calculations** - Automatically calculated based on buy-ins and cash-outs
- **Game duration** - Length of poker sessions

**Important:** We do NOT collect GPS coordinates, precise geolocation data, or physical addresses. Venue names are simple text descriptions that you manually enter.

### 2.4 Cash Game Auditor Data

When you use the Cash Game Auditor feature:

- **Player names** - Names you enter for tracking purposes
- **Buy-in records** - Individual player buy-in amounts
- **Cash-out records** - Individual player cash-out amounts
- **Settlement calculations** - Who owes whom
- **Audit timestamps** - When audits were created or modified
- **Exported documents** - When you download audit data as a document

### 2.5 Social Data

When you use social features:

- **Friend connections** - User IDs of friends you've added
- **Friend requests** - Sent and received requests
- **Leaderboard participation** - Your performance metrics visible to friends
- **Shared posts** - Content you post to your feed
- **Feed activity** - Games and audits you choose to share
- **Interactions** - Likes, comments, or engagement with friends' content (if applicable)

### 2.6 Device and Technical Information

We automatically collect certain technical information:

- **Device type and model** - iPhone model, iOS version
- **Operating system version** - iOS version number
- **Unique device identifiers** - Device-specific IDs for app functionality
- **Mobile carrier information** - Network provider
- **App version** - Which version of ChipChaser you're using
- **IP address** - For security and fraud prevention
- **Connection type** - WiFi or cellular data

This information is collected through the Expo and React Native frameworks that power the App.

### 2.7 Usage and Analytics Data

We collect information about how you use the App:

- **Session frequency** - How often you use the App
- **Feature usage** - Which features you access
- **Session duration** - How long you use the App
- **Navigation patterns** - Which screens you visit
- **Crash reports** - Error logs if the App malfunctions
- **Performance metrics** - App loading times and responsiveness

**Note:** We do NOT use third-party analytics services like Google Analytics or Facebook Analytics.

### 2.8 Agreement Acceptance Metadata

We store metadata about your acceptance of our Terms of Service and Privacy Policy:

- **Acceptance timestamp** - Date and time you agreed
- **Document version** - Which version of the agreements you accepted
- **IP address at acceptance** - For verification purposes

### 2.9 Information We Do NOT Collect

To be clear, we do NOT collect:

- Precise GPS coordinates or real-time location tracking
- Physical addresses of venues
- Credit card or payment information (the App is free)
- Biometric data (fingerprints, face scans)
- Microphone or audio recordings
- Contact lists from your device
- Text messages or call logs
- Photos from your device (unless you explicitly select one for your profile)
- Health or medical information
- Government identification numbers
- Information about your actual poker winnings or taxes

## 3. How We Use Your Information

### 3.1 To Provide and Improve Services

We use your information to:

- Create and manage your account
- Authenticate your identity and prevent unauthorized access
- Display your poker session history and statistics
- Calculate win/loss metrics and performance data
- Enable the Cash Game Auditor functionality
- Connect you with friends and display leaderboards
- Show your activity in friend feeds
- Store and display your profile picture
- Allow you to export and share audit documents
- Provide customer support and respond to inquiries

### 3.2 To Enable Social Features

We use your information to:

- Show your performance to friends on leaderboards
- Display your shared games in friend feeds
- Match friend requests with other users
- Show mutual friends and connections
- Enable sharing of cash game audits

### 3.3 To Maintain Security and Prevent Abuse

We use your information to:

- Detect and prevent fraud and unauthorized access
- Identify and ban users who violate our Terms of Service
- Monitor for suspicious activity or abuse
- Verify account ownership
- Protect against security threats

### 3.4 To Improve the App

We use aggregated, anonymized usage data to:

- Identify bugs and technical issues
- Understand which features are most popular
- Improve app performance and user experience
- Develop new features based on usage patterns
- Test new functionality

### 3.5 To Comply with Legal Obligations

We may use your information to:

- Respond to subpoenas or court orders
- Comply with applicable laws and regulations
- Cooperate with law enforcement when legally required
- Enforce our Terms of Service
- Protect our legal rights

### 3.6 What We Do NOT Use Your Information For

We do NOT:

- Sell your personal information to third parties
- Send marketing emails or promotional messages
- Use your data for advertising purposes
- Share your data with advertisers
- Track you across other websites or apps
- Use your information for AI training without permission

## 4. How We Share Your Information

### 4.1 With Your Friends (Your Choice)

When you add friends on ChipChaser, you explicitly choose to share:

- Your poker session performance (visible on leaderboards)
- Your profile information and picture
- Games and audits you choose to post to the feed
- Your username and basic profile details

You control what you share through privacy settings and by choosing what to post.

### 4.2 With Service Providers

We share your information with trusted third-party service providers who help us operate the App:

**Clerk (Authentication Provider)**
- **What they receive:** Email, username, password (encrypted), authentication tokens
- **Why:** To provide secure account authentication and login services
- **Their role:** Data processor acting on our behalf
- **Location:** United States
- **Their privacy policy:** [https://clerk.com/legal/privacy](https://clerk.com/legal/privacy)

**Amazon Web Services (AWS)**
- **What they receive:** All app data including sessions, profiles, pictures, and usage data
- **Why:** To host our backend infrastructure and store data
- **Data centers:** US-West-2 (Oregon region)
- **Their role:** Infrastructure provider and data processor
- **Security:** AWS complies with SOC 2, ISO 27001, and other security standards
- **Their privacy policy:** [https://aws.amazon.com/privacy/](https://aws.amazon.com/privacy/)

**Amazon S3 (File Storage)**
- **What they receive:** Profile pictures you upload
- **Why:** To securely store and serve images
- **Location:** US-West-2 (Oregon)
- **Security:** Encrypted storage with access controls

### 4.3 For Legal Reasons

We may disclose your information if required by law or in good faith belief that such action is necessary to:

- Comply with a legal obligation, court order, or subpoena
- Protect and defend our rights or property
- Prevent or investigate possible wrongdoing
- Protect the personal safety of users or the public
- Protect against legal liability
- Comply with law enforcement requests

We will notify you of such disclosures unless legally prohibited from doing so.

### 4.4 Business Transfers

If Bod Productions is involved in a merger, acquisition, asset sale, or bankruptcy, your information may be transferred to the new entity. We will notify you via email and/or a prominent notice in the App before your information is transferred and becomes subject to a different privacy policy.

### 4.5 With Your Consent

We may share your information for purposes not described in this Privacy Policy if we obtain your explicit consent.

### 4.6 Aggregated and Anonymized Data

We may share aggregated, anonymized data that cannot identify you personally:

- Overall app usage statistics
- General trends in poker tracking
- Feature popularity metrics

This data is used to improve the App and may be shared with partners or the public.

### 4.7 What We DO NOT Share

We do NOT:

- Sell your personal information to third parties
- Share your data with advertisers
- Share your data with data brokers
- Provide your email to marketing companies
- Share your poker performance with anyone other than your friends

## 5. Data Storage and Security

### 5.1 Where Your Data is Stored

Your data is stored on Amazon Web Services (AWS) cloud infrastructure in the **US-West-2 (Oregon)** region of the United States.

Specifically:
- **Backend services:** AWS Lambda serverless functions in Oregon
- **Database:** AWS-managed databases in Oregon
- **Profile pictures:** Amazon S3 storage buckets in Oregon
- **Authentication data:** Managed by Clerk in their secure infrastructure

### 5.2 Security Measures

We implement reasonable security measures to protect your information:

**Technical Safeguards:**
- Encryption in transit (TLS/SSL for all data transmission)
- Encryption at rest for stored data
- Secure password hashing (via Clerk)
- Regular security updates and patches
- Access controls and authentication requirements
- Firewalls and network security

**Organizational Safeguards:**
- Limited employee access to user data
- Security training and awareness
- Incident response procedures
- Regular security reviews

**AWS Security Features:**
- SOC 2, ISO 27001, and PCI DSS compliance
- DDoS protection
- Automated backups
- Infrastructure security monitoring

### 5.3 Security Limitations

While we strive to protect your information, no method of transmission or storage is 100% secure. We cannot guarantee absolute security. You use the App at your own risk.

**You are responsible for:**
- Keeping your password secure
- Not sharing your account credentials
- Logging out on shared devices
- Reporting suspicious activity

### 5.4 Data Retention

We retain your information for as long as your account is active or as needed to provide services. Specifically:

**Active Accounts:**
- Account data: Retained while account exists
- Session data: Retained indefinitely or until you delete
- Profile pictures: Retained until you change or delete them
- Audit data: Retained until you delete

**Deleted Accounts:**
- Most data is permanently deleted within 30 days
- Some data may be retained for legal compliance (e.g., fraud prevention records)
- Backups may retain data for up to 90 days before automatic deletion

**Legal Requirements:**
- We may retain certain data longer if required by law
- Financial records may be retained for tax purposes (if applicable)
- Dispute-related data may be retained until resolution

### 5.5 Data Backups

We maintain regular backups of data to prevent loss. Backup data is:
- Stored securely in AWS infrastructure
- Encrypted at rest
- Automatically deleted after 90 days
- Not accessible to users
- Used only for disaster recovery

## 6. Your Privacy Rights and Choices

### 6.1 Access Your Data

You have the right to access your personal information. You can:
- View your profile, sessions, and audits within the App
- Request a copy of your data by emailing memariaf21@gmail.com
- We will respond within 30 days

### 6.2 Correct Your Data

You have the right to correct inaccurate information:
- Update your profile information in the App
- Edit session data and audit records
- Contact us to correct information you cannot change yourself

### 6.3 Delete Your Data

You have the right to delete your account and data:

**Using the App:**
- Navigate to Settings → Account → Delete Account
- Confirm your decision
- Your account and data will be permanently deleted

**What Gets Deleted:**
- Your account and login credentials
- All poker session data
- Cash game auditor records
- Profile picture and profile information
- Friend connections
- Shared posts and feed activity

**What May Be Retained:**
- Metadata required for legal compliance (acceptance timestamps)
- Aggregated, anonymized analytics data
- Data in backups for up to 90 days
- Records of Terms of Service violations
- Information required by law

**Contact Us:**
Email memariaf21@gmail.com to request account deletion if you cannot access the App.

### 6.4 Export Your Data

You can export certain data from the App:
- Cash game auditor data as documents
- You can request a full data export by emailing memariaf21@gmail.com

### 6.5 Control Social Sharing

You can control what you share:
- Manage your friend list
- Remove friends at any time
- Choose what to post to your feed
- Adjust privacy settings in the App

### 6.6 Opt-Out Rights

Since we do not:
- Send marketing emails
- Use advertising tracking
- Share data with third-party marketers

There are no marketing opt-out choices required.

## 7. State-Specific Privacy Rights

### 7.1 California Residents (CCPA/CPRA)

If you are a California resident, you have additional rights under the California Consumer Privacy Act (CCPA) and California Privacy Rights Act (CPRA):

**Right to Know:**
You can request:
- Categories of personal information we collect
- Specific pieces of personal information we hold
- Categories of sources from which we collect information
- Our business purposes for collecting information
- Categories of third parties with whom we share information

**Right to Delete:**
You can request deletion of your personal information, subject to certain exceptions.

**Right to Correct:**
You can request correction of inaccurate personal information.

**Right to Opt-Out of Sale:**
We do NOT sell your personal information, so there is nothing to opt out of.

**Right to Non-Discrimination:**
We will not discriminate against you for exercising your privacy rights.

**How to Exercise Your Rights:**
Email us at memariaf21@gmail.com with "California Privacy Rights" in the subject line. We will respond within 45 days.

**Verification:**
We may need to verify your identity before fulfilling requests. We will ask for information to match against our records.

**Authorized Agents:**
You may designate an authorized agent to make requests on your behalf. The agent must provide proof of authorization.

### 7.2 Virginia Residents (VCDPA)

Virginia residents have similar rights to access, correct, delete, and obtain a copy of their personal information. Contact us at memariaf21@gmail.com to exercise these rights.

### 7.3 Other U.S. State Laws

Residents of Colorado, Connecticut, Utah, and other states with privacy laws may have similar rights. Contact us to exercise your rights under applicable state laws.

### 7.4 European Union / UK Residents (GDPR)

If you are in the EU or UK, you have rights under the General Data Protection Regulation (GDPR):

**Legal Basis for Processing:**
- **Contract performance:** To provide the App services
- **Legitimate interests:** To improve the App and prevent fraud
- **Consent:** For optional features like social sharing

**Your Rights:**
- Right to access your data
- Right to rectification (correction)
- Right to erasure ("right to be forgotten")
- Right to restrict processing
- Right to data portability
- Right to object to processing
- Right to withdraw consent

**International Data Transfers:**
Your data may be transferred to the United States. We rely on standard contractual clauses and adequacy decisions for lawful transfers.

**Supervisory Authority:**
You have the right to lodge a complaint with your local data protection authority.

**Contact for GDPR Requests:**
Email memariaf21@gmail.com with "GDPR Request" in the subject line.

## 8. Children's Privacy (COPPA Compliance)

ChipChaser is NOT intended for children under 18 years of age.

- We do not knowingly collect information from anyone under 18
- You must be at least 18 to create an account
- If we discover a user is under 18, we will immediately delete their account
- If you believe a child under 18 has provided information to us, contact us immediately at memariaf21@gmail.com

We comply with the Children's Online Privacy Protection Act (COPPA) and do not knowingly target or collect information from minors.

## 9. Third-Party Links and Services

### 9.1 Third-Party Websites

The App may contain links to third-party websites or services. We are not responsible for the privacy practices of these third parties. We encourage you to review their privacy policies before providing any information.

### 9.2 Clerk Authentication

Clerk is our authentication service provider. When you create an account, you agree to Clerk's terms and privacy policy. For more information:
- Website: [https://clerk.com](https://clerk.com)
- Privacy Policy: [https://clerk.com/legal/privacy](https://clerk.com/legal/privacy)

Clerk processes your authentication data on our behalf and is contractually obligated to protect your information.

### 9.3 Amazon Web Services (AWS)

AWS provides our cloud infrastructure. AWS acts as a data processor and is bound by strict security and privacy obligations. For more information:
- Website: [https://aws.amazon.com](https://aws.amazon.com)
- Privacy Policy: [https://aws.amazon.com/privacy/](https://aws.amazon.com/privacy/)

### 9.4 Expo Framework

The App is built using Expo and React Native. These frameworks may collect limited technical data for functionality purposes. We do not use Expo's analytics services.

## 10. International Data Transfers

ChipChaser operates in the United States, and your data is stored on servers located in Oregon (US-West-2).

If you access the App from outside the United States:
- Your information will be transferred to and processed in the United States
- U.S. privacy laws may differ from your country's laws
- By using the App, you consent to the transfer of your information to the U.S.

For users in the European Union, we rely on:
- Standard Contractual Clauses (SCCs) with service providers
- Adequacy decisions where applicable
- Your explicit consent for data transfers

## 11. Do Not Track Signals

Our App does not respond to "Do Not Track" (DNT) browser signals because:
- We do not track you across third-party websites
- We do not use advertising trackers
- We do not share data with advertising networks

## 12. Cookies and Local Storage

### 12.1 What We Use

The App may use local storage on your device to:
- Keep you logged in (authentication tokens)
- Store app preferences and settings
- Cache data for performance
- Remember your session state

This data is stored locally on your device and is not shared with third parties.

### 12.2 Essential Cookies

We use essential cookies/storage that are necessary for the App to function:
- Authentication tokens (via Clerk)
- Session management
- Security features

These cannot be disabled without preventing the App from working.

### 12.3 No Advertising or Tracking Cookies

We do NOT use:
- Advertising cookies
- Third-party tracking cookies
- Social media pixels
- Cross-site tracking

## 13. Data Breach Notification

In the event of a data breach that affects your personal information:

- We will investigate and contain the breach
- We will notify affected users within 72 hours of discovery
- Notification will be sent via email and/or in-app alert
- We will describe the nature of the breach and steps we're taking
- We will provide recommendations to protect yourself
- We will notify relevant regulatory authorities if required by law

If you suspect unauthorized access to your account, immediately:
1. Change your password
2. Contact us at memariaf21@gmail.com
3. Review your account activity

## 14. Changes to This Privacy Policy

### 14.1 Right to Modify

We may update this Privacy Policy from time to time to reflect:
- Changes in our data practices
- New features or services
- Legal or regulatory requirements
- User feedback

### 14.2 How We Notify You

When we make changes:
- We will update the "Last Updated" date at the top
- We will notify you through the App or via email
- Material changes may require your explicit consent
- You may be asked to review and accept the updated policy

### 14.3 Your Continued Use

Your continued use of the App after changes take effect constitutes acceptance of the updated Privacy Policy. If you do not agree with changes, you must stop using the App and delete your account.

### 14.4 Review Regularly

We encourage you to review this Privacy Policy periodically to stay informed about how we protect your information.

## 15. Contact Us

For questions, concerns, or requests regarding this Privacy Policy or your personal information:

**Bod Productions**  
**Email:** memariaf21@gmail.com

**For specific privacy requests, please include:**
- Your full name and username
- Email address associated with your account
- Nature of your request (access, deletion, correction, etc.)
- Any relevant details to help us verify your identity

**Response Time:**
We will respond to privacy requests within:
- **30 days** for general inquiries
- **45 days** for California residents (CCPA requests)
- **30 days** for EU/UK residents (GDPR requests)

We may extend these timeframes in complex cases and will notify you of any delay.

## 16. Additional Information

### 16.1 Data Controller

For purposes of data protection laws, Bod Productions is the data controller responsible for your personal information.

### 16.2 Data Protection Officer

Given the size and nature of our operations, we have not appointed a formal Data Protection Officer. Privacy inquiries should be directed to memariaf21@gmail.com.

### 16.3 Automated Decision-Making

We do not use automated decision-making or profiling that produces legal or similarly significant effects.

### 16.4 Sensitive Information

We do not intentionally collect sensitive personal information such as:
- Health or medical data
- Racial or ethnic origin
- Political opinions or religious beliefs
- Trade union membership
- Genetic or biometric data
- Sexual orientation

If you choose to include such information in free-text fields (like session notes), you do so at your own discretion.

---

## 17. Summary of Key Points

**What we collect:**
- Account info (email, username) via Clerk
- Poker session data you enter
- Profile pictures you upload
- Friends and social interactions
- Technical device information

**How we use it:**
- Provide tracking and social features
- Calculate performance metrics
- Enable friend connections and leaderboards
- Improve the App

**Who we share with:**
- Your friends (what you choose to share)
- Clerk (authentication)
- AWS (hosting)
- Legal authorities (when required)

**Your rights:**
- Access your data
- Correct inaccurate information
- Delete your account
- Export certain data
- Control social sharing

**Security:**
- Data stored in AWS Oregon
- Encryption in transit and at rest
- Regular security updates
- Limited access controls

**Contact us:**
memariaf21@gmail.com

---

## 18. Your Consent

By using ChipChaser, you acknowledge that:

1. You have read and understood this Privacy Policy
2. You consent to the collection, use, and sharing of your information as described
3. You understand your privacy rights and how to exercise them
4. You agree to the storage of your data in the United States
5. You are at least 18 years of age

Your consent is recorded with a timestamp when you accept this Privacy Policy in the App.

**This Privacy Policy was last updated on November 15, 2025 and is effective immediately.**

---

**© 2025 Bod Productions. All rights reserved.**
